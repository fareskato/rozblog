...
//    }
    ...
    private function _showStockInput($day, $month, $year) {
        ...
        $retour = "<div class='day-stock ".$stockClass."'>";
            ...
            if(count($stockAtDate['orders']) > 0) {
                $root_url = (strpos($_SERVER['DOCUMENT_ROOT'], 'devaltimarussia') !== false) ? '/feyarose' : '';
                $retour .= "<div class='stock-detail'><a href='".$root_url."/wp-content/plugins/feyarose-orders/feyaroseorders_export.php?date=".$start_date."'>View orders</a></div>";
            }
        $retour .= "</div>";
    ...
    private function _createNavi(){
        ...
        return
            ...
            '<span class="title">'.date('Y M',strtotime($this->currentYear.'-'.$this->currentMonth.'-1')).' - St.Petersburg - ' . $this->productID.'</span>'.
