		...
		if ( ! empty( $formatted_meta ) ) {
			...
			foreach ( $formatted_meta as $meta_key => $meta ) {
				...
				if ( $flat ) {
					...
					$meta_list[] = wp_kses_post( feyarose_getCleanNameOfProductAddon($meta['label'], true) . ': ' . $meta['value'] );
				} else {
                    /*
					$meta_list[] = '
                    ...
                    */
                    ...
                    $meta_list[] = '
							<dt class="variation-' . sanitize_html_class( sanitize_text_field( $meta_key ) ) . '">' . wp_kses_post( feyarose_getCleanNameOfProductAddon($meta['label'], true) ) . ':</dt>
							<dd class="variation-' . sanitize_html_class( sanitize_text_field( $meta_key ) ) . '">' . wp_kses_post( wpautop( make_clickable( $meta['value'] ) ) ) . '</dd>
						';
