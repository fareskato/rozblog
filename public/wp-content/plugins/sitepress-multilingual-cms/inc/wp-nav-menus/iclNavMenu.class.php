...
class iclNavMenu {
	...
	function nav_menu_language_controls() {
		...
		if ( $this->current_menu['id'] ) {
			...
			foreach ( $sitepress->get_active_languages() as $lang ) {
				if ( $lang['code'] == $this->current_menu['language'] ) {
					continue;
				}
	...
	function _set_custom_status_in_theme_location_switcher() {
		...
		if ( defined( 'ICL_IS_WPML_RESET' ) && ICL_IS_WPML_RESET ) {
			return;
		}
				...
				addLoadEvent(function () {
					...
					if (location_menu_id.length > 0) {
						...
						location_menu_id.change(function () {
							if (jQuery(this).val() != 0) jQuery(this).css('font-style', 'normal'); else jQuery(this).css('font-style', 'italic')
						});
					}
					<?php endforeach; ?>
				});
