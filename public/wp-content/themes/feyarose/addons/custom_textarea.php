<?php foreach ( $addon['options'] as $key => $option ) :
	...
	<p class="form-row form-row-wide addon-wrap-<?php echo sanitize_title( $addon['field-name'] ); ?> <?php echo feyarose_getCleanNameOfProductAddon($option['label']); ?>">
		...
		<?php endif; ?>	
		<textarea id="<?php echo feyarose_getCleanNameOfProductAddon($option['label']); ?>" placeholder="<?php echo feyarose_getCleanNameOfProductAddon($option['label'], true); ?>" type="text" class="input-text addon addon-custom-textarea" data-price="<?php echo get_product_addon_price_for_display( $option['price'] ); ?>" name="<?php echo $addon_key ?>[<?php echo $option_key; ?>]" rows="4" cols="20" <?php if ( ! empty( $option['max'] ) ) echo 'maxlength="' . $option['max'] .'"'; ?>><?php echo esc_textarea( $current_value ); ?></textarea>
	</p>
