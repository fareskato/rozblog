<?php foreach ( $addon['options'] as $key => $option ) :
	...
	<p class="form-row form-row-wide addon-wrap-<?php echo sanitize_title( $addon['field-name'] ); ?> <?php echo feyarose_getCleanNameOfProductAddon($option['label']); ?>">
		...
		<?php endif; ?>
		<input id="<?php echo feyarose_getCleanNameOfProductAddon($option['label']); ?>" type="number" step="any" class="input-text addon addon-custom-price" name="<?php echo $addon_key ?>[<?php echo $option_key; ?>]" value="<?php echo esc_attr( $current_value ); ?>" <?php if ( ! empty( $option['min'] ) ) echo 'min="' . $option['min'] .'"'; ?> <?php if ( ! empty( $option['max'] ) ) echo 'max="' . $option['max'] .'"'; ?> />
	</p>
